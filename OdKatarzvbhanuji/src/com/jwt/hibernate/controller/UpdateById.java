...
public class UpdateById extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
			...
			if (typ.equals("update")){
				writer.print("<title>Edycja uzytkownika</title></head>" 
							+ "<body> <center><table><tr><center>");
				writer.print("<th><form action='updateUser' method='post'>"
						+ "<input type='hidden' name='typ' value='update'/>"
						+ "<input type='hidden' name='currentSessionUser' value=" + userr + " />"
						+ "Podaj ID uzytkownika: </br><input name='id' value=''/>"
					+	"<input type='submit' value='Edytuj' class='okbutton' /></form></th>" );
			}
			else if (typ.equals("delete")){
				writer.print("<title>Usuwanie uzytkownika</title></head>" 
						+ "<body> <center><table><tr><center>");
			writer.print("<th><form action='deleteUser' method='post'>"
